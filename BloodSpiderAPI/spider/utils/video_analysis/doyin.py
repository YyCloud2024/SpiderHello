...
class DouyinVideoParse:
    ...
    def analysis(self, shared_link: str):
        ...
        video_page_url = requests.get(shared_link, headers=self.common_utils.get_headers("mobile"), allow_redirects=False)
        # 第二个
        video_data_text = requests.get(video_page_url.headers["Location"].split("?")[0][0:-1], headers=self.common_utils.get_headers("mobile"))
        # 使用正则表达式提取视频信息
        pattern = re.compile(
...
if __name__ == '__main__':
    ...
    print(douyin_video_parse.analysis("https://v.douyin.com/VPJqNfmlg68/"))
